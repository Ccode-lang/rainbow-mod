...
package net.mcreator.rainbow.world.biome;
...
public class BiomeRainbowBiome extends ElementsRainbowMod.ModElement {
	@GameRegistry.ObjectHolder("rainbow:rainbow_biome")
	public static final BiomeGenCustom biome = null;
	public BiomeRainbowBiome(ElementsRainbowMod instance) {
		super(instance, 21);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Rainbow Biome").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-16724992).setHeightVariation(0.2F)
					.setTemperature(1F));
			setRegistryName("rainbow_biome");
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntitySheep.class, 20, 4, 4));
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == Blocks.LEAVES2.getStateFromMeta(0).getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.LEAVES2.getStateFromMeta(0));
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.LEAVES2.getStateFromMeta(0).getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.LOG2.getStateFromMeta(0));
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.LOG2.getStateFromMeta(0).getBlock()
					|| blockType == Blocks.LEAVES2.getStateFromMeta(0).getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
